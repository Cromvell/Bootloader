...
void print_char(char character, int col, int row, char attribute_byte) {
    ...
    // If we see a new line character, set offset to the end of
    ...
    // Make scrolling adjustment, for when we reached the bottom
    // of the screen.
...
int get_cursor() {
    // This device uses its control register as an index
    // to select its internal registers, of which we are
    // interested in:
    //  reg 14: which is the high byte of the cursor's offset
    //  reg 15: which is the log byte of the cursor's offset
